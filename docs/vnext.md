...
*   **End-of-focus digest, glanceable then expandable.** The constellation's summary glow gains an optional click-through: a local, ephemeral list of what was held (source, tier, message), newest first, self-dismissing. Also a "while you were away" digest on unlock after >30 min.
...
*   **GitHub** (spec §4 🔭): Actions runs, review requests, mentions via REST + PAT; same shape as the GitLab connector.
...
*   **Teams presence sync (Microsoft Graph).** Read `Presence.Read` to treat *InACall / InAMeeting / DoNotDisturb* as focus-hold (note: Teams "Focusing" [surfaces as DoNotDisturb](https://learn.microsoft.com/en-us/answers/questions/1189978/presence-api-and-teams-focusing-status), which is exactly what we want). Optionally two-way: entering Periphery focus mode sets Teams DND so humans stop pinging too. Reuses the existing Graph auth.
...
*   **`periphery` CLI + MCP server.** The spec'd CLI sugar (`long_build && periphery done`), plus a local MCP server exposing `fire_cue` so coding agents can signal long-task completion ambiently instead of via terminal bells.
